...
"""
This script takes in  a sensor_msgs/Joy message from the Logitech Gamepad f710 and outputs either
...
ToDo (current bugs): 1. Brakes don't work. Just tapping/depressing lightly takes the speed to zero instantly.
        Should scale with amount pressed instead.
    2. Fix MPH, m/s inconsistency. Choose default and then convert.
    3. This node should handle x-mode and d-mode.
    4. Publish to the racing_cockpit. (Might need to create an intermediary message and then convert).
    5. Publish to New Eagle.  (Might need to create an intermediary message and then convert).
"""
...
class JoystickTeleopNode(object):
    """docstring for ClassName"""

    def __init__(self, ):
        """Constructor for JoystickTeleopNode"""
        super(JoystickTeleopNode, self).__init__()
        ...
        # joystick mappings. todo: confirm
        joystick_mode = rospy.get_param("~joystick_mode", "X").upper()
        if joystick_mode == "X":
            # buttons
            ...
            # self.LT_BUTTON = 1
            # self.RT_BUTTON = 1
            self.LEFT_STICK_BUTTON = 1
            self.RIGHT_STICK_BUTTON = 1
            self.BACK_BUTTON = 6
            self.START_BUTTON = 7
            # axes
            self.DIRECTION_SIDEWAYS = 6  # left>0.0, right<0.0
            self.DIRECTION_LONGITUDINAL = 7  # up>0.0, down<0.0
            self.LEFT_STICK_SIDEWAYS = 0
            self.LEFT_STICK_LONGITUDINAL = None  # todo
            self.RIGHT_STICK_SIDEWAYS = 3
            self.RIGHT_STICK_LONGITUDINAL = None  # todo
            self.LT = 2
            self.RT = 5
        elif joystick_mode == "D":
            rospy.loginfo("Only 'X' mode is supported. Switch to 'X' mode to continue.")
            self.shutdown()
        ...
        self.BTN_ENABLE = 5
        self.BTN_DISABLE = 4
        self.BTN_STEER_MULT_1 = 7  # start button
        self.BTN_ENABLE_AUTONOMOUS_MODE = 6  # back button
        self.BTN_COUNT = 11
    ...
    def joystick_callback(self, msg):
        ...
        # Speed increment. ToDo: change to axes button constant at the top
        if msg.axes[self.AXIS_SPEED_INCREMENT] != self.joy.axes[self.AXIS_SPEED_INCREMENT]:
            if msg.axes[self.AXIS_SPEED_INCREMENT] < -0.5:
                self.joystick_data['abs_speed_joy'] -= self.SPEED_INCREMENT
                if self.joystick_data['abs_speed_joy'] < 0:
                    self.joystick_data['abs_speed_joy'] = 0.0
            elif msg.axes[self.AXIS_SPEED_INCREMENT] > 0.5:
                self.joystick_data['abs_speed_joy'] += self.SPEED_INCREMENT
...
def main(args):
    ...
    joystick_teleop_instance = JoystickTeleopNode()
